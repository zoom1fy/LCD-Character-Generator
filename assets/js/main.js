...
function showWarningAlert(message, options = {}) {
  Swal.fire({
    title: translations[userLang].warning || 'Warning',
    text: translations[userLang][message] || message,
    icon: 'warning',
    iconColor: '#ffc107',
    confirmButtonText: translations[userLang].ok || 'OK',
    confirmButtonColor: '#3085d6',
    backdrop: true,
  });
}
...
async function copyToClipboard(elementId) {
  try {
    ...
    Swal.fire({
      icon: 'success',
      title: translations[userLang].success || 'Success!',
      text: translations[userLang].copy_success || 'Text copied to clipboard',
      timer: 2000,
    ...
    Swal.fire({
      icon: 'error',
      title: translations[userLang].error || 'Error',
      text: translations[userLang].copy_error || 'Failed to copy text',
    });
  }
}
